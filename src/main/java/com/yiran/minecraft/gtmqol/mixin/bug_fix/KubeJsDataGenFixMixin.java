...
@Restriction(
        require = @Condition(value = "kubejs", type = Condition.Type.MOD))
public class KubeJsDataGenFixMixin {
    @ModifyReturnValue(method = "isDataGen", at = @At("RETURN"), remap = false, require = 0)
    // set to 0 so that if this is fixed, this does nothing rather than crash the game
